...
CNmcDevice::CNmcDevice(CWnd* pParent)
{
	...
	m_pParamCtrl = &(((CEtherCat*)m_pParent)->m_ParamCtrl);
	m_pParamMotion = ((CEtherCat*)m_pParent)->m_pParamMotion;
	m_pParamAxis = ((CEtherCat*)m_pParent)->m_pParamAxis;
	m_pParamMotor = ((CEtherCat*)m_pParent)->m_pParamMotor;
...
double CNmcDevice::CalSpeedProfile(int nAxisID, double fLength, double &fVel, double &fAcc, double &fDec)
{
	...
	double fAccPeriod = ((CEtherCat*)m_pParent)->m_pParamAxis[nAxisID].fAccPeriod;			// m_fAccPeriod[nAxisID];
	double fAccLength;
	fAcc = ((CEtherCat*)m_pParent)->m_pParamAxis[nAxisID].fAcc;							// m_fAcc[nAxisID];

	if (fAcc <= 0.0 || fAcc > ((CEtherCat*)m_pParent)->m_pParamAxis[nAxisID].fMaxAccel)	// m_fMaxAccel[nAxisID])
	...
	{
		fVel = fAcc*fAccTime;
		if (fAccTime < ((CEtherCat*)m_pParent)->m_pParamAxis[nAxisID].fMinJerkTime)	// m_fMinJerkTime[nAxisID])
			fAccTime = ((CEtherCat*)m_pParent)->m_pParamAxis[nAxisID].fMinJerkTime;	// m_fMinJerkTime[nAxisID];
		fAcc = fVel / fAccTime;
		fDec = fAcc;
	}
